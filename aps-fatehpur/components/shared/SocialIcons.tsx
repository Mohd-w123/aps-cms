...
/* ── SVG social icons (inline, no external dependency) ── */
...
export const socialPlatforms = [
  ...
  { key: "website", label: "Website", color: "#6B7280" },
] as const;
...
export function SocialLinksBar({ links, size = "md", variant = "dark" }: SocialLinksBarProps) {
    ...
    <div className="flex gap-2 flex-wrap">
      {validPlatforms.map(p => (
        <a
          ...
          aria-label={p.label}
          className={`flex items-center justify-center rounded-full transition-colors ${sizeClass} ${bgClass}`}
          title={p.label}
        >
